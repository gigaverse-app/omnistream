...
describe("Stream Service", () => {
  ...
  describe("createStream", () => {
    ...
    it("should create streams on multiple platforms", async () => {
      ...
      expect(result.platformStreams.map((ps) => ps.platform)).toContain(
        Platform.YOUTUBE,
      );
      expect(result.platformStreams.map((ps) => ps.platform)).toContain(
        Platform.FACEBOOK,
  ...
  describe("startStream", () => {
    ...
    it("should throw NotFoundError if stream does not exist", async () => {
      await expect(
        streamService.startStream("invalid-id", community.id),
      ).rejects.toThrow(NotFoundError);
    });
    ...
    it("should throw NotFoundError if community does not own stream", async () => {
      ...
      await expect(
        streamService.startStream(stream.id, otherCommunity.id),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe("stopStream", () => {
    ...
    it("should throw NotFoundError if stream does not exist", async () => {
      await expect(
        streamService.stopStream("invalid-id", community.id),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe("getStreamStatus", () => {
    ...
    it("should throw NotFoundError if stream does not exist", async () => {
      await expect(
        streamService.getStreamStatus("invalid-id", community.id),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe("listStreams", () => {
    ...
    it("should throw NotFoundError if community does not exist", async () => {
      await expect(streamService.listStreams("invalid-id")).rejects.toThrow(
        NotFoundError,
      );
    });
  });
  ...
  describe("deleteStream", () => {
    ...
    it("should throw NotFoundError if stream does not exist", async () => {
      await expect(
        streamService.deleteStream("invalid-id", community.id),
      ).rejects.toThrow(NotFoundError);
    });
    ...
    it("should throw NotFoundError if community does not own stream", async () => {
      ...
      await expect(
        streamService.deleteStream(stream.id, otherCommunity.id),
      ).rejects.toThrow(NotFoundError);
    });
  });
});
